...
class DynamicObject():
    ...
    def create(cls, model_id: int, x: float, y: float, z: float, rotation_x: float, rotation_y: float, rotation_z: float, world_id: int = -1, interior_id: int = -1, player_id: int = -1, stream_distance: float = 300.0, draw_distance: float = 0.0, area_id: int = -1, priority = 0):
        return cls(create_dynamic_object(model_id, x, y, z, rotation_x, rotation_y, rotation_z, world_id, interior_id, player_id, stream_distance, draw_distance, area_id, priority), x, y, z, rotation_x, rotation_y, rotation_z, model_id)
    ...
    def move(self, x: float, y: float, z: float, speed: float, rotation_x: float = -1000.0, rotation_y: float = -1000.0, rotation_z: float = -1000.0):
        return move_dynamic_object(self.id, x, y, z, speed, rotation_x, rotation_y. rotation_z)
    ...
    def get_material_text(self) -> Tuple[int, int, int, int, int, int, int]:
        return self._material_size, self._material_color, self._font_size, self._bold, self._font_color, self._back_color, self._text_alignment
    ...
    def on_player_edit(cls, player_id: int, object_id: int, response, x: float, y: float, z: float, rotation_x: float, rotation_y: float, rotation_z: float):
        return call_native_function("OnPlayerEditDynamicObject", Player(player_id), cls(object_id), response, x, y, z, rotation_x, rotation_y, rotation_z)